...
namespace Jellyfin.Plugin.DoubanBookshelf.Common
{
    /// <summary>
    ...
    public static class BookFileNameParser
    {
        ...
        private static readonly Regex _trailingDoubanId = new(@"(?<!\d)(?<id>\d{6,9})$", RegexOptions.Compiled);
        ...
        /// <summary>
        /// Extracts a Douban subject id from the end of a Jellyfin book name.
        /// </summary>
        /// <param name="value">Book name.</param>
        /// <returns>The extracted Douban subject id, or null.</returns>
        public static string? GetTrailingDoubanId(string? value)
        {
            ...
            var match = _trailingDoubanId.Match(value.Trim());
            return match.Success ? match.Groups["id"].Value : null;
        }

        /// <summary>
        /// Removes a trailing Douban subject id from a Jellyfin book name.
        /// </summary>
        /// <param name="value">Book name.</param>
        /// <returns>The book name without the trailing Douban subject id.</returns>
        public static string RemoveTrailingDoubanId(string? value)
        {
            return string.IsNullOrWhiteSpace(value) ? string.Empty : _trailingDoubanId.Replace(value.Trim(), string.Empty).Trim();
        }
        ...
        {
            var nameParserResult = Parse(RemoveTrailingDoubanId(item.Name), item.SeriesName);

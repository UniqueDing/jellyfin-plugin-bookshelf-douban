...
namespace Jellyfin.Plugin.DoubanBookshelf.Tests
{
    public class BookFileNameParserTests
    {
        ...
        [Fact]
        public void GetTrailingDoubanId_WithDirtyJellyfinName_ReturnsDoubanId()
        {
            var result = BookFileNameParser.GetTrailingDoubanId("百年孤独马尔克斯指定译本无增删6564285");

            Assert.Equal("6564285", result);
        }

        [Fact]
        public void RemoveTrailingDoubanId_WithDirtyJellyfinName_RemovesDoubanId()
        {
            var result = BookFileNameParser.RemoveTrailingDoubanId("百年孤独马尔克斯指定译本无增删6564285");

            Assert.Equal("百年孤独马尔克斯指定译本无增删", result);
        }

        [Fact]
        public void GetBookMetadata_WithTrailingDoubanId_RemovesIdFromName()
        {
            var result = BookFileNameParser.GetBookMetadata(new BookInfo
            {
                Name = "百年孤独马尔克斯指定译本无增删6564285"
            });

            Assert.Equal("百年孤独马尔克斯指定译本无增删", result.Name);
        }
